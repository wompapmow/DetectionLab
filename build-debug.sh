...
print_usage() {
  echo "Usage: ./build.sh <virtualbox | vmware_desktop> [--workstations count] [--vagrant-only | --packer-only]"
  exit 0
}
...
check_vagrant_instances_exist() {
  ...
  VAGRANT_BUILT=$(vagrant status | grep '(' | grep -vc 'not[ _]created')  # look for entries with status indicated by parenthesis
  if [ "$VAGRANT_BUILT" -gt 0 ]; then
...
parse_cli_arguments() {
  # If no arguments supplied, print instructions
  if [ "$#" -eq 0 ]; then
  print_usage
  exit 1
  fi
  ...
  # If the user specifies the provider as an argument, set the variable
  # TODO: Check to make sure they actually have their provider installed
  case $key in
      virtualbox)  # can't be 'virtualbox|vmware_desktop)' due to disparity in values for PACKER_PROVIDER
      PROVIDER="$1"
      PACKER_PROVIDER="$1"
      shift # past argument
      ;;
      ...
      ;;
      --workstations)
      WS_COUNT="$2"  # TODO: Add validation that this is a numeric value
      shift # past argument
      shift # past value
      ;;
      --packer-only)
      ...
      shift # past argument
      ;;
      --vagrant-only)  # TODO: Add simple logic to see if both packer-only and vagrant-only are set, exit
      VAGRANT_ONLY=1
      shift # past argument
      ;;
      *)    # unknown option
      echo -e "\"$1\" is not a recognized option or provider. Available options are:\\n--workstations <count>\\n--packer-only\\n--vagrant-only"
      print_usage
      exit 1
      ;;
  esac
...
build_vagrant_hosts() {
  ...
  if [ -f "$DL_DIR/Vagrant/Vagrantfile.orig" ]; then
    cd "$DL_DIR/Vagrant"
    mv -f Vagrantfile{.orig,}
    cd - > /dev/null  # hide output; pushd/popd didn't work as expected
  fi
  
  ((WS_COUNT-=1))  # adjust for zero index
  if [ $WS_COUNT -ge 1 ]; then
    cd "$DL_DIR/Vagrant"
    sed -i.orig "s/0..0)/0..$WS_COUNT)/" Vagrantfile  # crossplatform safe, in-place replacement. have not tested w/ existing file
    cd - > /dev/null 
  fi
  
  COUNTER=0
  while [ $COUNTER -le $WS_COUNT ]; do
    LAB_HOSTS+=("win10-$COUNTER")
    ((COUNTER+=1))
  done
  # echo "${LAB_HOSTS[@]}" # DEBUG
